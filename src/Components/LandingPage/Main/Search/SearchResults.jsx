...
const Section = ({posters, key}) => (
  <section className="sectioned_posters" key={key}>{posters}</section>
);
...
const createSectionedPosters = (movieResults, setCurrentShowId, setShowType, setCurrentMenuItem, setHomepageLoaded) => {
  const postersMap = movieResults.map((info, idx) => {
    ...
    return <img className="search_posters" src={imageUrlBuilder(imagePath)} alt={title} onClick={onClick} key={idx}/>
  });
  return getSectionedPosters(postersMap);
};

export const SearchResults = ({currentShowTitle, setCurrentShowId, setCurrentShowType, homepageLoaded, setHomePageLoaded, setCurrentMenuItem}) => {
  ...
  useEffect(() => {
    ...
    fetchSearchedShow("tv", currentShowTitle, setTvResults, setLoaded, setHomePageLoaded);
  }, [currentShowTitle, setHomePageLoaded]);

  const ResultsComponent = ({minimizeMethod, minimized, anchorText, results, setShowType, setHomepageLoaded}) => {
    return <section className="show_items">
      ...
      {!minimized && <section className="show_results">
        {_.isEmpty(results)
          ? <span className="notice">No Results</span>
          : createSectionedPosters(results, setCurrentShowId, setCurrentShowType, setShowType, setCurrentMenuItem, setHomepageLoaded)}
  ...
  const Component = () => {
    return <section className="search_results">
      <span className="search_text">{`Search Results For ${currentShowTitle}...`}</span>
      <ResultsComponent minimizeMethod={minimizeMovies}
                        anchorText="Movies"
                        results={movieResults}
                        setShowType={setMovie} minimized={movieMinimized}
                        setHomepageLoaded={setHomePageLoaded}/>
      <ResultsComponent minimizeMethod={minimizeTv}
                        anchorText="TV Shows"
                        results={tvResults}
                        setShowType={setTv}
  ...
  return !loaded || !homepageLoaded ? <Spinner loaded={loaded}/> : <Component/>
}
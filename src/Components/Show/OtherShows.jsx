...
const createMiniShows = (otherShows, setCurrentShow) => {
    return otherShows.map((show,index) => {
...
const Component = ({className, otherShows, keyword, setCurrentShow}) => {
    return <section className={className}>
        <h3 className="section_title">{_.capitalize(keyword)}</h3>
        <section className="mini_shows">
            {createMiniShows(otherShows, setCurrentShow)}
        </section>
    </section>;
};
